...
There are a couple of ways to deploy a Gatsby site to GitHub pages ranging from naive approaches
to something that requires a bit of work but is the recommended way. In this tutorial I'll walk you
...
fast websites and apps. One of the primary use cases of Gatsby is Static Site Generation(SSG), which
makes it useful for deploying personal websites with more granular control.
...
again, it requires effort for something which is not the best practice. Which brings us to the
second approach.
...
Okay, so plenty of things are happening in this YAML file. First, this automated process is
performed on a **push** to the `main` branch or on a **pull request** is performed. If these events
occur, it then runs a job on the latest Ubuntu build, and performs installation, build and test
on 3 different node versions.
...
practice to write test cases for your project as it verifies that both your recent push
or PRs do not break your app's functionality.
...
in `Settings > Secrets > New repository secret` your GitHub repository. This can now be accessed through
